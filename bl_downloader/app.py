...
from bl_downloader.gui.login_window import LoginWindow
from bl_downloader.services.downloader import download_video
from bl_downloader.services.login import delete_credential, load_credential, save_credential
...
class App:
    ...
    def _build_ui(self) -> None:
        ...
        # row 0: login
        self.login_btn = ttk.Button(frame, text="登录", command=self._on_login)
        self.login_btn.grid(row=0, column=0, columnspan=3, sticky=tk.E, pady=(0, 4))
        _ToolTip(self.login_btn, "登录后可以下载高清视频")

        # row 1: URL
        ttk.Label(frame, text="视频链接:").grid(row=1, column=0, sticky=tk.W, pady=(0, 4))
        url_row = ttk.Frame(frame)
        url_row.grid(row=1, column=1, columnspan=2, sticky=tk.EW, pady=(0, 8))
        ...
        self.info_frame.grid(row=2, column=0, columnspan=3, sticky=tk.EW, pady=(0, 8))
        ...
        # row 3: quality
        ttk.Label(frame, text="清晰度:").grid(row=3, column=0, sticky=tk.W, pady=(0, 4))
        self.quality_var = tk.StringVar()
        self.quality_combo = ttk.Combobox(
            frame, textvariable=self.quality_var, state="readonly", width=30
        )
        self.quality_combo.grid(row=3, column=1, sticky=tk.W, pady=(0, 8))

        # row 4: save path
        ttk.Label(frame, text="保存到:").grid(row=4, column=0, sticky=tk.W, pady=(0, 4))
        path_row = ttk.Frame(frame)
        path_row.grid(row=4, column=1, columnspan=2, sticky=tk.EW, pady=(0, 8))
        ...
        self.download_btn = ttk.Button(btn_frame, text="开始", command=self._on_download, width=12)
        ...
        self.progress_bar.grid(row=6, column=0, columnspan=3, sticky=tk.EW, pady=(4, 2))

        # row 7: status
        self.status_var = tk.StringVar(value="就绪")
        self.status_label = ttk.Label(frame, textvariable=self.status_var)
        self.status_label.grid(row=7, column=0, columnspan=3, sticky=tk.W, pady=(0, 4))

        # row 8: log
        log_frame = ttk.LabelFrame(frame, text="日志", padding=4)
        log_frame.grid(row=8, column=0, columnspan=3, sticky=tk.NSEW, pady=(4, 0))
    ...
    def _update_login_ui(self) -> None:
        if self._credential is not None:
            ...
            self.status_var.set("已登录B站账号")
        else:
            self.login_btn.configure(text="登录")

    def _on_login(self) -> None:
        if self._credential is not None:
            if messagebox.askyesno("退出登录", "确定要退出当前账号吗？"):
                delete_credential()
        ...
        LoginWindow(
            parent=self.root,
            on_done=self._on_login_success,
        )

    def _on_login_success(self, credential: Credential) -> None:
    ...
    def _on_parse(self) -> None:
        ...
        def _fetch():
            async def _run():
                try:
                    info = await get_video_info(url, credential=self._credential)
